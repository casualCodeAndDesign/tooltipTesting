                ...
                _ExceptionDetails.prototype.parseStack = function (stack) {
                    ...
                    if (typeof stack === "string") {
                        ...
                        if (totalSizeInBytes > exceptionParsedStackThreshold) {
                            ...
                            var size = 0;
                            var acceptedbottom = bottom;
                            var acceptedRight = right;
                            while (bottom < right) {
                                var lSize = parsedStack[bottom].sizeInBytes;
                                var rSize = parsedStack[right].sizeInBytes;
                                size += lSize + rSize;
                                if (size > exceptionParsedStackThreshold) {
                                    var howMany = acceptedRight - acceptedbottom + 1;
                                    parsedStack.splice(acceptedbottom, howMany);
                                    break;
                                }
                                acceptedbottom = bottom;
                                acceptedRight = right;
                                bottom++;
                                right--;
                            }
